...
lSphere = P.sum . fmap sphere

llSphere :: [[Double]] -> Double
llSphere = P.sum . fmap (P.sum . fmap sphere)

fSphere :: Fold Double Double
fSphere = Fold (\x a -> x + (a - 1.0)*(a - 1.0)) 0.0 id

foldSphere :: Fold [Double] Double
foldSphere = Fold (\x a -> x + lSphere a) 0.0 id

ffSphere :: [[Double]] -> Double
ffSphere = fold foldSphere

vSphere :: V.Vector Double -> Double
vSphere = V.sum . V.map (\a -> (a - 1.0)*(a - 1.0))

vvSphere :: V.Vector (V.Vector Double) -> Double
vvSphere = V.sum . V.map (V.sum . V.map (\a -> (a - 1.0)*(a - 1.0)))

-- griew function
data Griew = Griew {_v1::Double, _v2::Double, _i1::Double}

fGriew :: Fold Double Double
fGriew = Fold (\(Griew v1 v2 i1) a ->
                Griew
                (v1+(a - 100.0) * (a - 100.0))
...
shekel xs = -(fold L.sum $ fmap (1.0/) (P.zipWith (+) (sp xs) shekelC))
          where
             sp a = fold L.sum <$> fmap (**2) <$> P.zipWith ($) ((-) <$> a) <$> shekelA
             -- rowf f r v = Prelude.zipWith ($) (f <$> r) <$> v

vShekel :: V.Vector Double -> Double
vShekel = fold fGriew
